...
class OpenCVTemplateMatchEngine:
    """OpenCV implementation of the detector-local matching port."""
    ...
    def match(
        ...
        *,
        ...
        if not isinstance(template, MatchTemplate):
            raise TypeError(
                "template must be MatchTemplate, "
        ...
        mask_input = (
            ...
            else np.ascontiguousarray(template.mask)
        )
        ...
        try:
            ...
            raise RuntimeError(
                "OpenCV template matching failed using "
